package client.conn_ctbsaldos_0_1;
...
/**
 * Job: conn_ctbsaldos Purpose: Conector SAP x SMART - Saldos Contabeis<br>
 * Description: Conector SAP x SMART - Saldos Contabeis <br>
 * @author sergio.paes@slaterit.com
 * @version 6.2.1.20160704_1411
 * @status 
 */
public class conn_ctbsaldos implements TalendJob {
	...
	public class ContextProperties extends java.util.Properties {
		...
		public void synchronizeContext() {
			...
			if (exercicio != null) {
				...
				this.setProperty("exercicio", exercicio.toString());
	...
	private final String jobName = "conn_ctbsaldos";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					conn_ctbsaldos.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(conn_ctbsaldos.this, new Object[] { e,
	...
	public static class row2Struct implements
		...
		final static byte[] commonByteArrayLock_client_conn_ctbsaldos = new byte[0];
		static byte[] commonByteArray_client_conn_ctbsaldos = new byte[0];

		public String SDCODIGOEMPRESA;

		public String getSDCODIGOEMPRESA() {
			return this.SDCODIGOEMPRESA;
		}

		public String SDCONTAANALITICA;

		public String getSDCONTAANALITICA() {
			return this.SDCONTAANALITICA;
		}

		public String SDDATASALDOINICIAL;

		public String getSDDATASALDOINICIAL() {
			return this.SDDATASALDOINICIAL;
		}

		public String SDVALORTOTALDEBITO;

		public String getSDVALORTOTALDEBITO() {
			return this.SDVALORTOTALDEBITO;
		}

		public String SDVALORTOTALCREDITO;

		public String getSDVALORTOTALCREDITO() {
			return this.SDVALORTOTALCREDITO;
		}

		public String SDVALORSALDOFINAL;

		public String getSDVALORSALDOFINAL() {
			return this.SDVALORSALDOFINAL;
		}

		public String SDINDICADORDCDBFINAL;

		public String getSDINDICADORDCDBFINAL() {
			return this.SDINDICADORDCDBFINAL;
		}
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_client_conn_ctbsaldos.length) {
					if (length < 1024
							&& commonByteArray_client_conn_ctbsaldos.length == 0) {
						commonByteArray_client_conn_ctbsaldos = new byte[1024];
					} else {
						commonByteArray_client_conn_ctbsaldos = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_client_conn_ctbsaldos, 0,
						length);
				strReturn = new String(
						commonByteArray_client_conn_ctbsaldos, 0, length,
						utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_client_conn_ctbsaldos) {

				try {

					int length = 0;

					this.SDCODIGOEMPRESA = readString(dis);

					this.SDCONTAANALITICA = readString(dis);

					this.SDDATASALDOINICIAL = readString(dis);

					this.SDVALORTOTALDEBITO = readString(dis);

					this.SDVALORTOTALCREDITO = readString(dis);

					this.SDVALORSALDOFINAL = readString(dis);

					this.SDINDICADORDCDBFINAL = readString(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeString(this.SDCODIGOEMPRESA, dos);

				// String

				writeString(this.SDCONTAANALITICA, dos);

				// String

				writeString(this.SDDATASALDOINICIAL, dos);

				// String

				writeString(this.SDVALORTOTALDEBITO, dos);

				// String

				writeString(this.SDVALORTOTALCREDITO, dos);

				// String

				writeString(this.SDVALORSALDOFINAL, dos);

				// String

				writeString(this.SDINDICADORDCDBFINAL, dos);
		...
		public String toString() {
			...
			sb.append("[");
			sb.append("SDCODIGOEMPRESA=" + SDCODIGOEMPRESA);
			sb.append(",SDCONTAANALITICA=" + SDCONTAANALITICA);
			sb.append(",SDDATASALDOINICIAL=" + SDDATASALDOINICIAL);
			sb.append(",SDVALORTOTALDEBITO=" + SDVALORTOTALDEBITO);
			sb.append(",SDVALORTOTALCREDITO=" + SDVALORTOTALCREDITO);
			sb.append(",SDVALORSALDOFINAL=" + SDVALORSALDOFINAL);
			sb.append(",SDINDICADORDCDBFINAL=" + SDINDICADORDCDBFINAL);
			sb.append("]");
	...
	public static class CTBSALDOSStruct implements
			routines.system.IPersistableRow<CTBSALDOSStruct> {
		final static byte[] commonByteArrayLock_client_conn_ctbsaldos = new byte[0];
		static byte[] commonByteArray_client_conn_ctbsaldos = new byte[0];
		 ...
		 * Compare keys
		 */
		public int compareTo(CTBSALDOSStruct other) {
	...
	public void tSAPInput_1Process(final java.util.Map<String, Object> globalMap)
		...
		try {
			...
			if (resumeEntryMethodName == null || resumeIt || globalResumeTicket) {// start
				...
				CTBSALDOSStruct CTBSALDOS = new CTBSALDOSStruct();
				...
				String insert_tMSSqlOutput_1 = "INSERT INTO ["
						+ tableName_tMSSqlOutput_1
						+ "] ([SDCODIGOEMPRESA],[SDCONTAANALITICA],[SDDATASALDOINICIAL],[SDVALORTOTALDEBITO],[SDVALORTOTALCREDITO],[SDVALORSALDOFINAL],[SDINDICADORDCDBFINAL]) VALUES (?,?,?,?,?,?,?)";
				...
				final class Helper_tReplaceNull_1 {
					...
					public void fill(outSMARTStruct input_row,
						...
						output_row.SDCODIGOEMPRESA = (String) coalesce(
								input_row.SDCODIGOEMPRESA, ' ', null, "");
						output_row.SDCONTAANALITICA = (String) coalesce(
								input_row.SDCONTAANALITICA, ' ', null, "");
						output_row.SDDATASALDOINICIAL = (String) coalesce(
								input_row.SDDATASALDOINICIAL, ' ', null, "");
						output_row.SDVALORTOTALDEBITO = (String) coalesce(
								input_row.SDVALORTOTALDEBITO, ' ', null, "");
						output_row.SDVALORTOTALCREDITO = (String) coalesce(
								input_row.SDVALORTOTALCREDITO, ' ', null, "");
						output_row.SDVALORSALDOFINAL = (String) coalesce(
								input_row.SDVALORSALDOFINAL, ' ', null, "");
						output_row.SDINDICADORDCDBFINAL = (String) coalesce(
								input_row.SDINDICADORDCDBFINAL, ' ', null, "");
					} // end of fill
				...
				com.sap.conn.jco.JCoFunctionTemplate functionTemplate_tSAPInput_1 = repository_tSAPInput_1
						.getFunctionTemplate("ZMSAF_CTB_SALDOS");
				...
				importParameterList_tSAPInput_1.setValue("I_EXERCICIO",
						context.exercicio); // "input_single"
				...
				boolean go_CTBSALDOS = true;

				com.sap.conn.jco.JCoTable table_CTBSALDOS_tSAPInput_1 = tableParameterList_tSAPInput_1
						.getTable("E_SALDOS");

				go_CTBSALDOS = !table_CTBSALDOS_tSAPInput_1.isEmpty();
				...
				go_CTBSALDOS) {
					...
					if (go_CTBSALDOS) {
						...
						if (table_CTBSALDOS_tSAPInput_1.isLastRow()) { // check
																		// the
																		// flag
																		// first
							go_CTBSALDOS = false;
						}
						CTBSALDOS = new CTBSALDOSStruct();

						// "table_output" or "output_table"--SDCODIGOEMPRESA

						CTBSALDOS.SDCODIGOEMPRESA = ParserUtils
								.parseTo_String(table_CTBSALDOS_tSAPInput_1
										.getString("SDCODIGOEMPRESA"));

						// "table_output" or "output_table"--SDCONTAANALITICA

						CTBSALDOS.SDCONTAANALITICA = ParserUtils
								.parseTo_String(table_CTBSALDOS_tSAPInput_1
										.getString("SDCONTAANALITICA"));

						// "table_output" or "output_table"--SDDATASALDOINICIAL

						CTBSALDOS.SDDATASALDOINICIAL = ParserUtils
								.parseTo_String(table_CTBSALDOS_tSAPInput_1
										.getString("SDDATASALDOINICIAL"));

						// "table_output" or "output_table"--SDVALORTOTALDEBITO

						CTBSALDOS.SDVALORTOTALDEBITO = ParserUtils
								.parseTo_String(table_CTBSALDOS_tSAPInput_1
										.getString("SDVALORTOTALDEBITO"));

						// "table_output" or "output_table"--SDVALORTOTALCREDITO

						CTBSALDOS.SDVALORTOTALCREDITO = ParserUtils
								.parseTo_String(table_CTBSALDOS_tSAPInput_1
										.getString("SDVALORTOTALCREDITO"));

						// "table_output" or "output_table"--SDVALORSALDOFINAL

						CTBSALDOS.SDVALORSALDOFINAL = ParserUtils
								.parseTo_String(table_CTBSALDOS_tSAPInput_1
										.getString("SDVALORSALDOFINAL"));

						// "table_output" or
						// "output_table"--SDINDICADORDCDBFINAL

						CTBSALDOS.SDINDICADORDCDBFINAL = ParserUtils
								.parseTo_String(table_CTBSALDOS_tSAPInput_1
										.getString("SDINDICADORDCDBFINAL"));

						if (go_CTBSALDOS) {
							table_CTBSALDOS_tSAPInput_1.nextRow();
						}
					...
					if (CTBSALDOS != null) {
						...
						{ // start of Var scope
							...
							outSMART_tmp.SDCODIGOEMPRESA = "001";
							outSMART_tmp.SDCONTAANALITICA = CTBSALDOS.SDCONTAANALITICA;
							outSMART_tmp.SDDATASALDOINICIAL = CTBSALDOS.SDDATASALDOINICIAL;
							outSMART_tmp.SDVALORTOTALDEBITO = CTBSALDOS.SDVALORTOTALDEBITO;
							outSMART_tmp.SDVALORTOTALCREDITO = CTBSALDOS.SDVALORTOTALCREDITO;
							outSMART_tmp.SDVALORSALDOFINAL = CTBSALDOS.SDVALORSALDOFINAL;
							outSMART_tmp.SDINDICADORDCDBFINAL = CTBSALDOS.SDINDICADORDCDBFINAL;
						...
						if (outSMART != null) {
							...
							String searchStr_tReplace_1_1 = "0120" + "";
							row1.SDDATASALDOINICIAL = StringUtils
									.replaceAllStrictly(
											row1.SDDATASALDOINICIAL,
											searchStr_tReplace_1_1, "012018"
													+ "", true, false);
							String searchStr_tReplace_1_2 = "0220" + "";
							row1.SDDATASALDOINICIAL = StringUtils
									.replaceAllStrictly(
											row1.SDDATASALDOINICIAL,
											searchStr_tReplace_1_2, "022018"
													+ "", true, false);
							String searchStr_tReplace_1_3 = "0320" + "";
							row1.SDDATASALDOINICIAL = StringUtils
									.replaceAllStrictly(
											row1.SDDATASALDOINICIAL,
											searchStr_tReplace_1_3, "032018"
													+ "", true, false);
							String searchStr_tReplace_1_4 = "0420" + "";
							row1.SDDATASALDOINICIAL = StringUtils
									.replaceAllStrictly(
											row1.SDDATASALDOINICIAL,
											searchStr_tReplace_1_4, "042018"
													+ "", true, false);
							String searchStr_tReplace_1_5 = "0520" + "";
							row1.SDDATASALDOINICIAL = StringUtils
									.replaceAllStrictly(
											row1.SDDATASALDOINICIAL,
											searchStr_tReplace_1_5, "052018"
													+ "", true, false);
							String searchStr_tReplace_1_6 = "0620" + "";
							row1.SDDATASALDOINICIAL = StringUtils
									.replaceAllStrictly(
											row1.SDDATASALDOINICIAL,
											searchStr_tReplace_1_6, "062018"
													+ "", true, false);
							String searchStr_tReplace_1_7 = "0720" + "";
							row1.SDDATASALDOINICIAL = StringUtils
									.replaceAllStrictly(
											row1.SDDATASALDOINICIAL,
											searchStr_tReplace_1_7, "072018"
													+ "", true, false);
							String searchStr_tReplace_1_8 = "0820" + "";
							row1.SDDATASALDOINICIAL = StringUtils
									.replaceAllStrictly(
											row1.SDDATASALDOINICIAL,
											searchStr_tReplace_1_8, "082018"
													+ "", true, false);
							String searchStr_tReplace_1_9 = "0920" + "";
							row1.SDDATASALDOINICIAL = StringUtils
									.replaceAllStrictly(
											row1.SDDATASALDOINICIAL,
											searchStr_tReplace_1_9, "092018"
													+ "", true, false);
							String searchStr_tReplace_1_10 = "1020" + "";
							row1.SDDATASALDOINICIAL = StringUtils
									.replaceAllStrictly(
											row1.SDDATASALDOINICIAL,
											searchStr_tReplace_1_10, "102018"
													+ "", true, false);
							String searchStr_tReplace_1_11 = "1120" + "";
							row1.SDDATASALDOINICIAL = StringUtils
									.replaceAllStrictly(
											row1.SDDATASALDOINICIAL,
											searchStr_tReplace_1_11, "112018"
													+ "", true, false);
							String searchStr_tReplace_1_12 = "1220" + "";
							row1.SDDATASALDOINICIAL = StringUtils
									.replaceAllStrictly(
											row1.SDDATASALDOINICIAL,
											searchStr_tReplace_1_12, "122018"
													+ "", true, false);
							row2.SDCODIGOEMPRESA = row1.SDCODIGOEMPRESA;

							row2.SDCONTAANALITICA = row1.SDCONTAANALITICA;

							row2.SDDATASALDOINICIAL = row1.SDDATASALDOINICIAL;

							row2.SDVALORTOTALDEBITO = row1.SDVALORTOTALDEBITO;

							row2.SDVALORTOTALCREDITO = row1.SDVALORTOTALCREDITO;

							row2.SDVALORSALDOFINAL = row1.SDVALORSALDOFINAL;

							row2.SDINDICADORDCDBFINAL = row1.SDINDICADORDCDBFINAL;
	...
	public static void main(String[] args) {
		final conn_ctbsaldos conn_ctbsaldosClass = new conn_ctbsaldos();

		int exitCode = conn_ctbsaldosClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = conn_ctbsaldos.class
					.getClassLoader().getResourceAsStream(
							"client/conn_ctbsaldos_0_1/contexts/"
			...
			context.exercicio = (String) context.getProperty("exercicio");
		...
		if (parentContextMap != null && !parentContextMap.isEmpty()) {
			...
			if (parentContextMap.containsKey("exercicio")) {
				context.exercicio = (String) parentContextMap.get("exercicio");
			}
		}
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory)
					+ " bytes memory increase when running : conn_ctbsaldos");
		}
...
/************************************************************************************************
 * 79240 characters generated by Talend Open Studio for Data Integration on the
 * 28 de Fevereiro de 2019 13h38min29s BRT
 ************************************************************************************************/
