package client.conn_ccusto_0_1;
...
/**
 * Job: conn_ccusto Purpose: Conector SAP x SMART - Centro de Custos<br>
 * Description: Conector SAP x SMART - Centro de Custos <br>
 * @author sergio.paes@slaterit.com
 * @version 6.2.1.20160704_1411
 * @status 
 */
public class conn_ccusto implements TalendJob {
	...
	private final String jobName = "conn_ccusto";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(conn_ccusto.this, new Object[] { e,
	...
	public static class row1Struct implements
		...
		final static byte[] commonByteArrayLock_client_conn_ccusto = new byte[0];
		static byte[] commonByteArray_client_conn_ccusto = new byte[0];

		public String tacodigo;

		public String getTacodigo() {
			return this.tacodigo;
		}

		public String taclassificacao;

		public String getTaclassificacao() {
			return this.taclassificacao;
		}

		public String tadescricao;
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_client_conn_ccusto.length) {
					if (length < 1024
							&& commonByteArray_client_conn_ccusto.length == 0) {
						commonByteArray_client_conn_ccusto = new byte[1024];
					} else {
						commonByteArray_client_conn_ccusto = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_client_conn_ccusto, 0, length);
				strReturn = new String(commonByteArray_client_conn_ccusto,
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_client_conn_ccusto) {

				try {

					int length = 0;

					this.tacodigo = readString(dis);

					this.taclassificacao = readString(dis);

					this.tadescricao = readString(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeString(this.tacodigo, dos);

				// String

				writeString(this.taclassificacao, dos);

				// String

				writeString(this.tadescricao, dos);
		...
		public String toString() {
			...
			sb.append("[");
			sb.append("tacodigo=" + tacodigo);
			sb.append(",taclassificacao=" + taclassificacao);
			sb.append(",tadescricao=" + tadescricao);
			sb.append("]");
	...
	public static class CCUSTOStruct implements
			routines.system.IPersistableRow<CCUSTOStruct> {
		final static byte[] commonByteArrayLock_client_conn_ccusto = new byte[0];
		static byte[] commonByteArray_client_conn_ccusto = new byte[0];
		 ...
		 * Compare keys
		 */
		public int compareTo(CCUSTOStruct other) {
	...
	public void tSAPInput_1Process(final java.util.Map<String, Object> globalMap)
		...
		try {
			...
			if (resumeEntryMethodName == null || resumeIt || globalResumeTicket) {// start
				...
				CCUSTOStruct CCUSTO = new CCUSTOStruct();
				...
				if (dbschema_tMSSqlOutput_1 == null
					...
					tableName_tMSSqlOutput_1 = "ztemp_ccusto";
				} else {
				...
				String insert_tMSSqlOutput_1 = "INSERT INTO ["
						+ tableName_tMSSqlOutput_1
						+ "] ([tacodigo],[taclassificacao],[tadescricao]) VALUES (?,?,?)";
				...
				final class Helper_tReplaceNull_1 {
					...
					public void fill(outSMARTStruct input_row,
						...
						output_row.tacodigo = (String) coalesce(
								input_row.tacodigo, ' ', null, "");
						output_row.taclassificacao = (String) coalesce(
								input_row.taclassificacao, ' ', null, "");
						output_row.tadescricao = (String) coalesce(
								input_row.tadescricao, ' ', null, "");
					} // end of fill
				...
				com.sap.conn.jco.JCoFunctionTemplate functionTemplate_tSAPInput_1 = repository_tSAPInput_1
						.getFunctionTemplate("ZMSAF_TAB_CCUSTO");
				...
				importParameterList_tSAPInput_1.setValue("I_KOKRS", "RIBR"); // "input_single"
				...
				boolean go_CCUSTO = true;

				com.sap.conn.jco.JCoTable table_CCUSTO_tSAPInput_1 = tableParameterList_tSAPInput_1
						.getTable("T_CCUSTO");

				go_CCUSTO = !table_CCUSTO_tSAPInput_1.isEmpty();
				...
				go_CCUSTO) {
					...
					if (go_CCUSTO) {
						...
						if (table_CCUSTO_tSAPInput_1.isLastRow()) { // check the
																	// flag
																	// first
							go_CCUSTO = false;
						}
						CCUSTO = new CCUSTOStruct();

						// "table_output" or "output_table"--tacodigo

						CCUSTO.tacodigo = ParserUtils
								.parseTo_String(table_CCUSTO_tSAPInput_1
										.getString("TACODIGO"));

						// "table_output" or "output_table"--taclassificacao

						CCUSTO.taclassificacao = ParserUtils
								.parseTo_String(table_CCUSTO_tSAPInput_1
										.getString("TACLASSIFICACAO"));

						// "table_output" or "output_table"--tadescricao

						CCUSTO.tadescricao = ParserUtils
								.parseTo_String(table_CCUSTO_tSAPInput_1
										.getString("TADESCRICAO"));

						if (go_CCUSTO) {
							table_CCUSTO_tSAPInput_1.nextRow();
						}
					...
					if (CCUSTO != null) {
						...
						{ // start of Var scope
							...
							outSMART_tmp.tacodigo = CCUSTO.tacodigo;
							outSMART_tmp.taclassificacao = CCUSTO.taclassificacao;
							outSMART_tmp.tadescricao = CCUSTO.tadescricao;
						...
						if (outSMART != null) {
							...
							if (row1.tacodigo == null) {
								pstmt_tMSSqlOutput_1.setNull(1,
										java.sql.Types.VARCHAR);
							} else {
								pstmt_tMSSqlOutput_1
										.setString(1, row1.tacodigo);
							}

							if (row1.taclassificacao == null) {
								pstmt_tMSSqlOutput_1.setNull(2,
										java.sql.Types.VARCHAR);
							} else {
								pstmt_tMSSqlOutput_1.setString(2,
										row1.taclassificacao);
							}

							if (row1.tadescricao == null) {
	...
	public static void main(String[] args) {
		final conn_ccusto conn_ccustoClass = new conn_ccusto();

		int exitCode = conn_ccustoClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = conn_ccusto.class.getClassLoader()
					.getResourceAsStream(
							"client/conn_ccusto_0_1/contexts/" + contextStr
									+ ".properties");
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory)
					+ " bytes memory increase when running : conn_ccusto");
		}
...
/************************************************************************************************
 * 61099 characters generated by Talend Open Studio for Data Integration on the
 * 28 de Fevereiro de 2019 13h37min38s BRT
 ************************************************************************************************/
